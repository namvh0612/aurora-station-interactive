...
The report is read one chapter at a time: who was on the panel, what the night asked of you, five readings one at a time, what feeds and what checks, how the record was made, and what you leave for the next watch. All six chapters are built into the page; the pager decides which is shown, puts the chapter in the address bar, and works with the browser's own back and forward buttons. A short orientation sits before the first chapter, and the closing record carries the method, the limits and the attribution. The watch itself can be restarted at any point from the masthead, which clears the record and keeps your sound setting. On a phone the station controls collapse behind a single button.
...
The report reads five currents — Wood, Fire, Earth, Metal and Water — one for each domain. Each is a line with a name at both ends, and a reading is a position on that line rather than a score out of five: The Orchard or The Wildwood, The Lantern or The Sun, The Rampart or The Cropland, The Gold or The Blade, The Deep Well or The Open Sea. Neither end is a shortage of the other, and both carry their own contribution and their own cost.

Two readings come off each line and are kept apart. **Which pole** the position falls on is decided only by which side of the middle it sits; **how pronounced** it is, is the distance from that middle. Folding one into the other is what makes a spectrum read as more-is-better. Nothing joins the five into a single label: five lines with two ends each would be thirty-two combinations, and naming them is the difference between a reflection and a type indicator.

The elements also supply the colour and the shape of one chapter: which current feeds another, and which holds another in check. It is used for that relationship and nothing else. Every reading comes from the sixty recorded responses; no part of it is derived from a date of birth. Each element keeps one hue across the whole product, set light enough to read on the night and deep enough to read on paper.

Negative Emotionality is read as itself on the Water line — more feeling in motion sits toward The Open Sea. It is never inverted into a differently-named reading. The five domain names appear once, in the closing record, where the attribution needs something to attach to.
